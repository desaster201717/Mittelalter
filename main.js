...
if (G.phase === ‘PEACE’) {
secs.textContent = Math.ceil(G.waveTimer);
wLabel.textContent = ’Angriff in ’ + Math.ceil(G.waveTimer) + ‘s’;
...
wLabel.textContent = ’Banditen übrig: ’ + remaining;
...
document.getElementById(‘bldgIcon’).textContent  = def.icon;
document.getElementById(‘bldgName’).textContent  = def.name;
...
document.getElementById(‘hpText’).textContent    = `HP: ${Math.ceil(b.hp)} / ${b.maxHp}`;
